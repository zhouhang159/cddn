...
class WPJAM_List_Table extends WP_List_Table{
	...
	public function call_model_method($method, ...$args){
		...
		if(method_exists($this->model, $method)){
			...
			$exists		= $static ? true : method_exists($this->model, 'get_instance');
	...
	public function get_row_action($action, $args=[]){
		...
		if(!empty($action['redirect'])){
			...
			$class		= in_array($action['key'],['move', 'move_item']) ? 'list-table-move-action' : 'list-table-action';
		...
		return empty($args['wrap']) ? $row_action : sprintf($args['wrap'], esc_attr($action['key']), $row_action);
	}
	...
	private function generate_data_attr($action, $args=[]){
		...
		if($args['type'] == 'button'){
			foreach(['direct', 'confirm'] as $action_attr){
	...
	public function render_column_items($id, $items, $args=[]){
		...
		if($item_type == 'image'){
			...
			foreach($items as $i=>$item){
				...
				$image		= wpjam_get_list_table_row_action('move_item',	array_merge($args, ['class'=>'move-item', 'title'=>$image]));
				$image		.= wpjam_get_list_table_row_action('del_item',	array_merge($args, ['class'=>'del-item-icon dashicons dashicons-no-alt', 'title'=>' ']));

				$title		= !empty($item['title']) ? '<span class="item-title" style="width:'.$width.'px;">'.$item['title'].'</span>' : '';

				$actions	= wpjam_get_list_table_row_action('move_item',	array_merge($args, [
					'class'	=>'move-item dashicons dashicons-move', 
					'title'	=>' ', 
					'wrap'	=>'<span class="%1$s">%2$s | </span>'
				])).wpjam_get_list_table_row_action('edit_item',	array_merge($args, [
					'class'	=>'', 
					'title'	=>'修改', 
					'wrap'	=>'<span class="%1$s">%2$s</span>'
				]));
			...
			if(empty($max_items) || $i < $max_items-1){
				$rendered	.= wpjam_get_list_table_row_action('add_item', [
					'tag'	=> 'div',
					'id'	=> $id,
			...
			foreach($items as $i=>$item){
				...
				$text		= $item[$key] ?: ' ';
				$text		= wpjam_get_list_table_row_action('move_item', array_merge($args, ['class'=>'move-item text',	'title'=>$text]));
				...
				$actions	= wpjam_get_list_table_row_action('move_item', array_merge($args, [
					...
					'title'	=> '修改',
					'wrap'	=> '<span class="%1$s">%2$s | </span>'
				])).wpjam_get_list_table_row_action('del_item', array_merge($args, [
					'title'	=> '删除',
			...
			if(empty($max_items) || $i < $max_items-1){
				$rendered	.= wpjam_get_list_table_row_action('add_item', ['id'=>$id,	'title'=>'新增']);
			}
		}

		$style	= $per_row ? ' style="width:'.($per_row * ($width+30)).'px;"' : '';
	...
	public function ajax_response(){
		...
		if($action_type == 'form'){
			...
			$width	= $action['width'] ?? 720;

			$this->send_json(array_merge($response, ['form'=>$ajax_form, 'width'=>$width]));
		...
		$result_as_response	= is_array($result) && (isset($result['bulk']) || isset($result['ids']) || isset($result['id']) || isset($result['items']));
		...
		if($action['response'] == 'append'){
			...
			$response['width']	= $action['width'] ?? 720;
			...
			if(!isset($action['update_row']) || $action['update_row']){
				if($result_as_response){
					$bulk	= $result['bulk'] ?? $bulk;
					$ids	= $result['ids'] ?? $ids;
					$id		= $result['id'] ?? $id;
				}

				if($response['type'] == 'items' && isset($response['items'])){
					foreach($response['items'] as $id => &$response_item){
						$response_item['id']	= $id;

						if(!in_array($response_item['type'], ['delete', 'append'])){
							if($id || is_numeric($id)){
								ob_start();
								$this->single_row($id);
								$response_item['data']	= ob_get_clean();
							}
						}
					}
					...
					if($bulk){
						...
						if($this->layout == 'calendar'){
							...
							if(!$result_as_response && in_array($action['response'], ['add', 'duplicate'])){
								if(is_array($result)){
	...
	protected function filter_fields($fields, $key, $id){
		...
		$fields	= apply_filters('wpjam_list_table_action_fields', $fields, $key, $id, $this->singular);
	...
	public function prepare_items(){
		...
		if(method_exists($this->model, 'query_data')){
			$result	= $this->call_model_method('query_data', ['number'=>$per_page, 'offset'=>$offset]);
		}else{
	...
	public function _js_vars() {
		// printf("<script type='text/javascript'>wpjam_page_setting.list_args = %s;</script>\n", wpjam_json_encode($list_args));
