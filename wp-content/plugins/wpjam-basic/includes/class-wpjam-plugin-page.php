...
class WPJAM_Menu_Page{
	...
	public  static function admin_enqueue_scripts(){
		...
		$params	= wpjam_array_except($_REQUEST, ['page', 'tab', 'post_type', 'taxonomy', '_wp_http_referer', '_wpnonce']);
		...
		if(!empty($GLOBALS['wpjam_list_table'])){
			...
			if($sortable = $GLOBALS['wpjam_list_table']->sortable){
				$setting['list_table']['sortable']	= $sortable === true ? ['items'=>' >tr'] : $sortable;
			}
			...
			$setting['list_table']	= $setting['list_table'] ?: new stdClass;
		}
...
class WPJAM_Page_Action{
	...
	public function current_user_can($type=''){
		$capability	= $this->capability ?? ($type == 'submit' ? 'manage_options' : 'read');

		return current_user_can($capability, $this->name);
	}

	public function callback(){
		if(!$this->current_user_can('submit')){
	...
	public function get_form($args=[]){
		...
		$args['fields']	= apply_filters('wpjam_page_action_fields', $args['fields'], $this->name);
