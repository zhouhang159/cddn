...
class WPJAM_DB{
	...
	*/
	...
	public function where(...$args){
		if(count($args) == 2){
			...
			foreach($args as $column => $value){
				$this->where_compare($column, $value);
			}
			return $this;
		}	
	}
	...
	public function where_compare($column, $value){
		...
		$type		= $value['type'] ?? null;	// 以后实现
		$value		= $value['value'] ?? '';

		if(!in_array($compare, $non_numeric_operators, true) && !in_array($compare, $numeric_operators, true)){
			$compare	= '=';
		}

		if($compare == '='){
			return $this->where($column, $value['value']);
		}elseif($compare == '!='){
			return $this->where_not($column, $value['value']);
		}elseif($compare == '>'){
			return $this->where_gt($column, $value['value']);
		}elseif($compare == '>='){
			return $this->where_gte($column, $value['value']);
		}elseif($compare == '<'){
			return $this->where_lt($column, $value['value']);
		}elseif($compare == '<='){
			return $this->where_lte($column, $value['value']);
		}elseif($compare == 'IN'){
			return $this->where_in($column, $value['value']);
		}elseif($compare == 'NOT IN'){
			return $this->where_not_in($column, $value['value']);
		}elseif($compare == 'LIKE'){
			return $this->where_like($column, $value['value']);
		}elseif($compare == 'NOT LIKE'){
			return $this->where_not_like($column, $value['value']);
		}elseif($compare == 'RLIKE' || $compare == 'REGEXP'){
			// to do 
		}elseif($compare == 'NOT REGEXP'){
			// to do 
